...
export default function ConfirmBookingScreen() {
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            ...
            try {
                ...
                const [chargersList, vehiclesList, priceResult] = await Promise.all([
                    getChargersByStationId(Number(stationId)),
                    getAllVehicleApi().then(res => res.data || res),
                    getActivePriceSetting(Number(stationId)).catch(() => null),
                ]);

                setAllChargers(chargersList);
                setPriceSetting(priceResult);
    ...
    // Use the station owner's real booking fee per hour; fall back to 0 if pricing isn't configured yet.
    const bookingFeePerHour = priceSetting?.bookingFee ?? 0;
    const priceEstimation = bookingFeePerHour * durationHours;
    ...
    useEffect(() => {
        const acquireLock = async () => {
            ...
            try {
                ...
                // Construct Date in local time, then convert to UTC for backend
