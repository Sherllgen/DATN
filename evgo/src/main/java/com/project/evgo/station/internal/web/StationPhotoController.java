...
public class StationPhotoController {
    ...
    @Operation(summary = "Add photo", description = "Add a new photo to a station (owner only)")
    public ResponseEntity<ApiResponse<StationPhotoResponse>> addPhoto(
            @PathVariable Long stationId,
            @Valid @RequestBody AddStationPhotoRequest request) {
        StationPhotoResponse result = stationPhotoService.addPhoto(stationId, request);
    ...
    public ResponseEntity<ApiResponse<List<StationPhotoResponse>>> reorderPhotos(
        ...
        List<StationPhotoResponse> result = stationPhotoService.reorderPhotos(stationId, request.photoIdsInOrder());
        return ResponseEntity.ok(ApiResponse.<List<StationPhotoResponse>>builder()
                .status(HttpStatus.OK.value())
